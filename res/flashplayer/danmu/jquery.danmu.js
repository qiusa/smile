...
(function ($) {
    var Danmu = function (element, options) {
        ...
        var nowSecCount = 0;
        //格式控制
        this.rowCount = parseInt(me.height / options.FontSizeBig) -11;
        ...
        this.$timer.timer({
            ...
            callback: function (index) {
                setTimeout(function () {
                    ...
                    me.rowCount = parseInt(me.height / options.FontSizeBig) - 11;
                    setTimeout(function(){
                        me.checkRow(me)
                    }, 0);
                    //字幕保护
                    ...
                    if ($(element).data("danmuList")[$(element).data("nowTime")] && me.launched.indexOf($(element).data("nowTime")) < 0) {
                        ...
                        for (var i = (danmus.length - 1); i >= 0; i--) {
                            ...
                            if(danmaku.isPic){
                                $("#" + me.id + "tempDanmaku").html(unescape(danmaku.text)).css({
                                    "color": danmaku.color
                                ...
                                $("#" + me.id + "tempDanmaku").text(unescape(danmaku.text))
                                .css({
                                    "color": danmaku.color
                                    , "text-shadow": " 0px 0px 2px #000000"
                                    , "-moz-opacity": $(element).data("opacity")
                            ...
                            if (danmaku.hasOwnProperty('isnew') && danmaku.isSelf) {
                            ...
                            if (danmaku.position == 0) {
                                ...
                                if (nowCount <= maxCount && nowSecCount <= maxCountPerSec) {
                                    ...
                                    danmaku["row"] = row;
                                    var baseTop = 10;
                                    var top_local = baseTop + (row) * options.FontSizeBig;
                                    ...
                                    $("#" + flyTmpName).css({
                                        ...
                                        , "position": "absolute"
                                        , "top": top_local+20*row
                                        , "left": left_local
                                    });
                                    var newSpeed = ($(element).width()+40)/me.speed;
                                    nowCount++;
                                    nowSecCount++;
                                    $("#" + flyTmpName).animate({left: -($("#" + flyTmpName).width() + 40)}, newSpeed
                                        , function () {
                                            $(this).remove();
                                            nowCount--;
                                            nowSecCount--;
    ...
    Danmu.prototype.danmuInit = function () {
    ...
    Danmu.prototype.addDanmu = function (arg) {
        if (arg instanceof Array) {
            ...
            if (this.$element.data("danmuList")[arg.time]) {
                if(this.$element.data("danmuList")[arg.time].length>5){
